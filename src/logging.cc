...
// This is a helper method for getting a formatted numeric output (basically
// having the commas which makes them easier to read)
...
const char *getDateString(uint32_t _t) {
  ...
  sprintf(scratch, "%4d-%02d-%02d", gtm->tm_year + 1900, gtm->tm_mon + 1,
          gtm->tm_mday);
  return scratch;
}
...
// This is a helper method to handle logging the output from scanning the
// blockchain
