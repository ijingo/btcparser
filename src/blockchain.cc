 ...
 * following is the original comments from original source code.
 */
...
//
// Source contained in this project includes portions of source code from other
// open source projects; though that source may have
// been modified to be included here.  Original notices are left in where
// appropriate.
//
// Some of the hash and bignumber implementations are based on source code find
// in the 'cbitcoin' project; though it has been modified here to remove all
// memory allocations.
//
// http://cbitcoin.com/
//
...
// These defines set the limits this parser expects to ever encounter on the
// blockchain data stream.
// In a debug build there are asserts to make sure these limits are never
// exceeded.
...
#define MAX_BLOCK_SIZE \
  (1024 * 1024) * 32  // never expect to have a block larger than 32mb
#define MAX_BLOCK_TRANSACTION \
  32768  // never expect more than 32768 transactions per block.
...
#define MAX_REASONABLE_SCRIPT_LENGTH \
  (1024 * 32)  // would never expect any script to be more than 16k in size;
               // that would be very unusual!
#define MAX_REASONABLE_INPUTS \
  32678  // really can't imagine any transaction ever having more than 32768
         // inputs
#define MAX_REASONABLE_OUTPUTS \
  32768  // really can't imagine any transaction ever having more than 32768
         // outputs
...
static bool gReportTransactionHash = true;
...
class Hash256 {
 public:
  ...
  inline bool operator==(const Hash256 &h) const {
    return mWord0 == h.mWord0 && mWord1 == h.mWord1 && mWord2 == h.mWord2 &&
           mWord3 == h.mWord3;
  }
...
class FileLocation : public Hash256 {
 public:
  FileLocation(void) {}
  FileLocation(const Hash256 &h, uint32_t fileIndex, uint32_t fileOffset,
               uint32_t fileLength, uint32_t transactionIndex)
      : Hash256(h) {
...
struct BlockPrefix {
  ...
  uint8_t mPreviousBlock[32];  // The 32 byte (256 bit) hash of the previous
...
class BlockImpl : public BlockChain::Block {
 public:
  ...
  // Return the current stream pointer representing a 32byte hash and advance
  // the read pointer accordingly
  ...
  inline uint32_t readVariableLengthInteger(void) {
    ...
    /*
    if (v <
        0xFD)  // If it's less than 0xFD use this value as the unsigned integer
    {
      ret = (uint32_t)v;
    } else {
      uint16_t v = readU16();
      if (v < 0xFFFF) {
        ...
        if (v < 0xFFFFFFFF) {
          ...
          assert(0);  // never expect to actually encounter a 64bit integer in
                      // the block-chain stream; it's outside of any reasonable
                      // expected value
          uint64_t v = readU64();
          ret = (uint32_t)v;
        }
    ...
    return ret;
    */
    // jingo: the original implementation seems diverge from the protocol
    if (v < 0xfd) {
      return static_cast<uint32_t>(v);
    } else if (v == 0xfd) {
      return static_cast<uint32_t>(readU16());
    } else if (v == 0xfe) {
      return static_cast<uint32_t>(readU32());
    } else {
      assert(0);
      return 0;
    }
  ...
  // Get the current read buffer address and advance the stream buffer by this
  // length; used to get the address of input/output scripts
  ...
  bool readInput(BlockChain::BlockInput &input) {
    ...
    input.responseScriptLength =
        readVariableLengthInteger();  // read the length of the script
    assert(input.responseScriptLength < MAX_REASONABLE_SCRIPT_LENGTH);

    if (input.responseScriptLength >= 8192) {
      logMessage(
          "Block: %d : Unreasonably large input script length of %d bytes.\r\n",
          gBlockIndex, input.responseScriptLength);
    }

    if (input.responseScriptLength < MAX_REASONABLE_SCRIPT_LENGTH) {
      input.responseScript =
          input.responseScriptLength
              ? getReadBufferAdvance(input.responseScriptLength)
              : NULL;  // get the script buffer pointer; and advance the read
                       // location
      input.sequenceNumber = readU32();
    } else {
      logMessage(
          "Block %d : Outrageous sized input script of %d bytes!  Shutting "
          "down.\r\n",
          gBlockIndex, input.responseScriptLength);
      exit(1);
    }
  ...
  void getAsciiAddress(BlockChain::BlockOutput &o) {
    ...
    // If this is a multi-sig address, *then* we need to generate a multisig
    // address for it.
  ...
  bool readOutput(BlockChain::BlockOutput &output) {
    ...
    if (output.challengeScriptLength >= 8192) {
      logMessage(
          "Block %d : Unreasonably large output script length of %d bytes.\r\n",
    ...
    output.challengeScript =
        output.challengeScriptLength
            ? getReadBufferAdvance(output.challengeScriptLength)
            : NULL;  // get the script buffer pointer and advance the read
                     // location

    if (output.challengeScript) {
      uint8_t lastInstruction =
          output.challengeScript[output.challengeScriptLength - 1];
      if (output.challengeScriptLength == 67 &&
          output.challengeScript[0] == 65 &&
          output.challengeScript[66] == OP_CHECKSIG) {
        output.publicKey[0] = output.challengeScript + 1;
        output.keyType = BlockChain::KT_UNCOMPRESSED_PUBLIC_KEY;
      }
      if (output.challengeScriptLength == 40 &&
          output.challengeScript[0] == OP_RETURN) {
        output.publicKey[0] = &output.challengeScript[1];
        output.keyType = BlockChain::KT_STEALTH;
      } else if (output.challengeScriptLength == 66 &&
                 output.challengeScript[65] == OP_CHECKSIG) {
        output.publicKey[0] = output.challengeScript;
        output.keyType = BlockChain::KT_UNCOMPRESSED_PUBLIC_KEY;
      } else if (output.challengeScriptLength == 35 &&
                 output.challengeScript[34] == OP_CHECKSIG) {
        output.publicKey[0] = &output.challengeScript[1];
        output.keyType = BlockChain::KT_COMPRESSED_PUBLIC_KEY;
      } else if (output.challengeScriptLength == 33 &&
                 output.challengeScript[0] == 0x20) {
        output.publicKey[0] = &output.challengeScript[1];
        ...
        logMessage(
            "WARNING: Unusual but expected output script. Block %s : "
            "Transaction: %s : OutputIndex: %s\r\n",
            formatNumber(gBlockIndex), formatNumber(gTransactionIndex),
            formatNumber(gOutputIndex));
        gIsWarning = true;
      } else if (lastInstruction == OP_CHECKMULTISIG &&
                 output.challengeScriptLength > 25)  // looks to be a multi-sig
      {
        const uint8_t *scanBegin = output.challengeScript;
        const uint8_t *scanEnd =
            &output.challengeScript[output.challengeScriptLength - 2];
        ...
        if (expectedPrefix && expectedPostfix) {
          ...
          while (keyIndex < 5 && scanBegin < scanEnd) {
            if (*scanBegin == 0x21) {
              ...
              output.multiSigFormat |=
                  bitMask;  // turn this bit on if it is in compressed format
              keyIndex++;
        ...
        if (output.publicKey[0] == NULL) {
          logMessage(
              "****MULTI_SIG WARNING: Unable to decipher multi-sig output. "
              "Block %s : Transaction: %s : OutputIndex: %s\r\n",
              formatNumber(gBlockIndex), formatNumber(gTransactionIndex),
              formatNumber(gOutputIndex));
          gIsWarning = true;
        }
      } else {
        // Ok..we are going to scan for this pattern.. OP_DUP, OP_HASH160, 0x14
        // then exactly 20 bytes after 0x88,0xAC
        // 25...
        if (output.challengeScriptLength > 25) {
          ...
          for (uint32_t i = 0; i < endIndex; i++) {
            ...
            if (scan[0] == OP_DUP && scan[1] == OP_HASH160 && scan[2] == 20 &&
                scan[23] == OP_EQUALVERIFY && scan[24] == OP_CHECKSIG) {
              output.publicKey[0] = &scan[3];
              output.keyType = BlockChain::KT_RIPEMD160;
              logMessage(
                  "WARNING: Unusual output script. Block %s : Transaction: %s "
                  ": OutputIndex: %s\r\n",
                  formatNumber(gBlockIndex), formatNumber(gTransactionIndex),
                  formatNumber(gOutputIndex));
              gIsWarning = true;
    ...
    if (!output.publicKey[0]) {
      ...
      output.keyType = BlockChain::KT_RIPEMD160;
      logMessage(
          "WARNING: Failed to decode public key in output script. Block %s : "
          "Transaction: %s : OutputIndex: %s scriptLength: %s\r\n",
          formatNumber(gBlockIndex), formatNumber(gTransactionIndex),
          formatNumber(gOutputIndex),
          formatNumber(output.challengeScriptLength));
      gReportTransactionHash = true;
      gIsWarning = true;
    }
    ...
    switch (output.keyType) {
      case BlockChain::KT_RIPEMD160:
        BitcoinRIPEMD160ToAddress(output.publicKey[0],
                                  output.addresses[0].address);
        break;
      case BlockChain::KT_SCRIPT_HASH:
        BitcoinRIPEMD160ToScriptAddress(output.publicKey[0],
                                        output.addresses[0].address);
        break;
      case BlockChain::KT_STEALTH:
        BitcoinRIPEMD160ToAddress(output.publicKey[0],
        ...
        break;
      case BlockChain::KT_UNCOMPRESSED_PUBLIC_KEY: {
        BitcoinPublicKeyToAddress(output.publicKey[0],
                                  output.addresses[0].address);
      } break;
      case BlockChain::KT_COMPRESSED_PUBLIC_KEY: {
        BitcoinCompressedPublicKeyToAddress(output.publicKey[0],
                                            output.addresses[0].address);
      } break;
      case BlockChain::KT_TRUNCATED_COMPRESSED_KEY: {
        uint8_t key[33];
      ...
      case BlockChain::KT_MULTISIG: {
        for (uint32_t i = 0; i < MAX_MULTISIG; i++) {
          ...
          if (output.multiSigFormat & mask) {
            BitcoinCompressedPublicKeyToAddress(output.publicKey[i],
                                                output.addresses[i].address);
          } else {
            BitcoinPublicKeyToAddress(output.publicKey[i],
                                      output.addresses[i].address);
          }
        }
      } break;
  ...
  bool readTransaction(BlockChain::BlockTransaction &transaction,
                       uint32_t &transactionIndex, uint32_t tindex) {
    bool ret = false;

    const uint8_t *transactionBegin = mBlockRead;

    transaction.transactionVersionNumber = readU32();  // read the transaction
                                                       // version number; always
                                                       // expect it to be 1

    if (transaction.transactionVersionNumber == 1 ||
        transaction.transactionVersionNumber == 2) {
    } else {
      gIsWarning = true;
      logMessage(
          "Encountered unusual and unexpected transaction version number of "
          "[%d] for transaction #%d\r\n",
      ...
      if (totalOutputCount < MAX_BLOCK_OUTPUTS) {
...

        {
          transaction.transactionLength =
              (uint32_t)(mBlockRead - transactionBegin);
          transaction.fileIndex = fileIndex;
          transaction.fileOffset =
              fileOffset + (uint32_t)(transactionBegin - mBlockData);
          transaction.transactionIndex = transactionIndex;
          transactionIndex++;
          computeSHA256(transactionBegin, transaction.transactionLength,
                        transaction.transactionHash);
          computeSHA256(transaction.transactionHash, 32,
                        transaction.transactionHash);
  ...
  // Step #5 : We read a 'bits' field; internal use defined by the bitcoin
  // software
  // Step #6 : We read the 'nonce' value; a randum number generated during the
  // mining process.
  ...
  //			: (d) Read the response script data; parsed using the
  // bitcoin
  // scripting system; a little virtual machine.
  //			: Read the sequence number.
  //			: Read the number of outputs
  // Step #8b : For/Each Output
  //			: (a) Read the value of the output in BTC fixed decimal;
  // see
  // docs.
  ...
  bool processBlockData(const void *blockData, uint32_t blockLength,
    ...
    transactionCount =
        readVariableLengthInteger();  // Read the number of transactions
    ...
    if (transactionCount < MAX_BLOCK_TRANSACTION) {
      ...
      for (uint32_t i = 0; i < transactionCount; i++) {
        ...
        if (!readTransaction(b, transactionIndex, i))  // Read the transaction;
                                                       // if it failed; then
                                                       // abort processing the
                                                       // block chain
        {
          ret = false;
          break;
        } else {
          logMessage("read txn data! %d\n", i);
        }
      }
    }
  ...
  const BlockChain::BlockTransaction *processTransactionData(
      const void *transactionData, uint32_t transactionLength) {
    uint32_t transactionIndex = 0;
    BlockChain::BlockTransaction *ret = &mTransactions[0];
    mBlockData = (const uint8_t *)transactionData;
    mBlockRead = mBlockData;  // Set the block-read scan pointer.
    mBlockEnd =
        &mBlockData[transactionLength];  // Mark the end of block pointer

    if (!readTransaction(*ret, transactionIndex, 0))  // Read the transaction;
                                                      // if it failed; then
                                                      // abort processing the
                                                      // block chain
    {
  ...
  BlockChain::BlockTransaction
      mTransactions[MAX_BLOCK_TRANSACTION];  // Holds the array of transactions
...
class BlockChainImpl : public BlockChain {
 public:
  ...
  BlockChainImpl(const char *rootDir, uint32_t maxBlocks) {
    ...
    mFileLength = 0;
    mCurrentBlockData =
        mBlockDataBuffer;  // scratch buffer to read up to 3 blocks
  ...
  virtual ~BlockChainImpl(void) {
    ...
    for (FILEVector::iterator i = mBlockDataFiles.begin();
         i != mBlockDataFiles.end(); ++i) {
      FILE *f = (*i);
      fclose(f);
    }
    if (mTextReport) {
  ...
  // Initial scan of the blockchain to build the hash table of valid blocks;
  // skipping orphan blocks
  // Will return 'true' if the scan is complete.  'lastBlockRead' is assigned
  // the number of blocks we have processed so far.
  ...
  // Initial scan of the blockchain to build the hash table of blocks in forward
  // order.
  // Contrary to what you might think, or expect, the blocks in the file are not
  // in the order of
  // 0,1,2,3,4 etc.  The reason for this is that sometimes, while the client is
  // connected to the network, orphan blocks get written
  // out.  So, the only way to know the correct blockchain is to sequentally
  // scan to the last block found.  For each block we compute it's
  // hash and store it into a hash-table.  Then, to build the correct version of
  // the blockchain we have to walk the linked-list backwards from
  ...
  bool readBlockHeader(void) {
    ...
    if (mBlockIndex < mBlockDataFiles.size()) {
      ...
      // Attempt to read the 'magicid' which we expect to see at the start of
      // each block
      size_t r = fread(&magicID, sizeof(magicID), 1,
                       fph);  // Attempt to read the magic id for the next block
      if (r == 0) {
        if (openBlock())  // Attempt to open the next block, if successful, look
                          // for the magicID in it.
        {
          fph = mBlockDataFiles[mBlockIndex];
          r = fread(&magicID, sizeof(magicID), 1, fph);  // if we opened up a
                                                         // new file; read the
                                                         // magic id from it's
                                                         // first block.
      ...
      // If after reading the previous block, we did not encounter a block
      // header, we need to scan for the next block header..
      if (r == 1 && magicID != MAGIC_ID) {
        ...
        if (c > 0) {
          for (uint32_t i = 0; i < c; i++) {
            ...
            if (*check == MAGIC_ID) {
              logMessage(
                  "Found the next block header after skipping: %s bytes "
                  "forward in the file.\r\n",
                  formatNumber(i));
        ...
        if (found)  // if we found it before the EOF, we are cool, otherwise, we
                    // need to advance to the next file.
        {
        } else {
          if (openBlock()) {
            fph = mBlockDataFiles[mBlockIndex];
            ...
            if (r == 1) {
              if (magicID != MAGIC_ID) {
                logMessage(
                    "Advanced to the next data file; but it does not start "
                    "with a valid block.  Aborting reading the "
                    "block-chain.\r\n");
                r = 0;
              }
            }
      ...
      if (r == 1)  // Ok, this is a valid block, let's continue
      {
        ...
        r = fread(&header.mBlockLength, sizeof(header.mBlockLength), 1,
                  fph);  // read the length of the block
        header.mFileOffset = (uint32_t)ftell(fph);
        if (r == 1) {
          assert(header.mBlockLength < MAX_BLOCK_SIZE);  // make sure the block
                                                         // length does not
                                                         // exceed our maximum
                                                         // expected ever
                                                         // possible block size
          if (header.mBlockLength < MAX_BLOCK_SIZE) {
            r = fread(&prefix, sizeof(prefix), 1, fph);  // read the rest of the
                                                         // block (less the 8
                                                         // byte header we have
                                                         // already consumed)
            if (r == 1) {
  ...
  // Opens the FILE associated with the next section of blocks (blk?????.dat)
  // sequence
  bool openBlock(void) {
    bool ret = false;

    mBlockIndex = uint32_t(
        mBlockDataFiles.size());  // this is which one we are trying to open...
    char scratch[512];
    sprintf(scratch, "%s/blk%05d.dat", mRootDir.c_str(),
            mBlockIndex);  // get the filename
  ...
  // Once scanning is completed, we build the blockchain by traversing, as a
  // linked list, the last block to every previous block.
  // This will allow it to successfully skip orphaned blocks.  Return value is
  // the last valid block number encountered.
  ...
  virtual bool readBlock(BlockImpl &block, uint32_t blockIndex) {
    ...
    if (fph) {
      ...
      size_t r = fread(blockData, block.blockLength, 1,
                       fph);  // read the rest of the block (less the 8 byte
                              // header we have already consumed)

      if (r == 1) {
        computeSHA256(blockData, 4 + 32 + 32 + 4 + 4 + 4,
                      block.computedBlockHash);
        computeSHA256(block.computedBlockHash, 32, block.computedBlockHash);
        ret = block.processBlockData(blockData, block.blockLength,
                                     mTransactionCount);

        /*
        if (mSearchForText)  // if we are searching for ASCII text in the input
                             // stream...
        {
              ...
              if (textCount == 0) {
                ...
                if (mTextReport) {
                  fprintf(mTextReport,
                          "==========================================\r\n");
                  fprintf(mTextReport,
                          "= ASCII TEXT REPORT for Block #%s on %s\r\n",
                          formatNumber(blockIndex),
  ...
  virtual const BlockTransaction *processSingleTransaction(
      const void *transactionData, uint32_t transactionLength) {
    ...
    if (transactionLength < MAX_BLOCK_SIZE) {
      ...
      ret = mSingleTransactionBlock.processTransactionData(transactionData,
                                                           transactionLength);
    }
    return ret;
  }

  virtual const BlockTransaction *readSingleTransaction(
    ...
    if (fileIndex < mBlockDataFiles.size() && mBlockDataFiles[fileIndex] &&
        transactionLength < MAX_BLOCK_SIZE) {
      ...
      fseek(fph, saveLocation, SEEK_SET);  // restore the file position back to
  ...
  virtual void printBlock(const Block *block)  // prints the contents of the
                                               // block to the console for
                                               // debugging purposes
  {
    logMessage(
        "======================================================================"
    ...
    logMessage("Size: %0.10f KB or %s bytes.\r\n",
               (float)block->blockLength / 1024.0f,
               formatNumber(block->blockLength));
    ...
    for (uint32_t i = 0; i < block->transactionCount; i++) {
      ...
      logMessage("Transaction %s : %s inputs %s outputs. VersionNumber: %d\r\n",
                 formatNumber(i), formatNumber(t.inputCount),
                 formatNumber(t.outputCount), t.transactionVersionNumber);
      ...
      for (uint32_t i = 0; i < t.inputCount; i++) {
        ...
        logMessage(
            "    Input %s : ResponsScriptLength: %s TransactionIndex: %s : "
            "TransactionHash: ",
            formatNumber(i), formatNumber(input.responseScriptLength),
            formatNumber(input.transactionIndex));
        ...
        if (input.transactionIndex != 0xFFFFFFFF) {
          const BlockTransaction *t =
              readSingleTransaction(input.transactionHash);
          if (t == NULL) {
            logMessage(
                "ERROR: TransactionIndex[%d] FAILED TO LOCATE TRANSACTION FOR "
                "HASH: ",
            ...
            if (input.transactionIndex < t->outputCount) {
              ...
              if (o.publicKey[0]) {
                logMessage(
                    "     Spending From Public Key: %s in the amount of: "
                    "%0.4f\r\n",
                    o.asciiAddress, (float)o.value / ONE_BTC);
              } else {
                logMessage(
                    "ERROR: No public key found for this previous output.\r\n");
              }
            } else {
      ...
      for (uint32_t i = 0; i < t.outputCount; i++) {
        ...
        logMessage("    Output: %s : %f BTC : ChallengeScriptLength: %s\r\n",
                   formatNumber(i), (float)output.value / ONE_BTC,
                   formatNumber(output.challengeScriptLength));
        if (output.publicKey[0]) {
          ...
          logMessage(
              "ERROR: Unable to derive a public key for this output!\r\n");
        }
      }
    }
    ...
    for (uint32_t i = 0; i < block.transactionCount; i++) {
      ...
      for (uint32_t j = 0; j < t.inputCount; j++) {
        ...
        if (input.transactionIndex != 0xFFFFFFFF) {
          ...
          if (gReportTransactionHash) {
            logMessage("try to find #%d txn, txn hash:", i);
            printReverseHash(input.transactionHash);
            logMessage("\n");
          }
          ...
          if (found == mTransactionSet.end()) {
            block.warning = true;
            logMessage("Failed to find input transaction!\r\n");
            exit(1);
          }
        }
        */
  ...
  uint32_t mScanCount;  // How many blocks we have processed in the 'forward'
                        // scan step.
  uint32_t mReadCount;
  ...
  BlockHeader *
      mBlockChainHeaders;  // Headers for every single block in the blockchain
...
BlockChain *BlockChain::createBlockChain(const char *rootPath,
                                         uint32_t maxBlocks) {
  BLOCK_CHAIN::BlockChainImpl *b =
      new BLOCK_CHAIN::BlockChainImpl(rootPath, maxBlocks);
